<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="{ row }">
          <span>{{ row.checkDate | parseTime("{y}-{m}-{d}") }}</span>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="{ row }">
          <el-tag type="primary" v-if="row.workDay">{{
            row.workDay ? "是" : "否"
          }}</el-tag>
          <el-tag type="danger" v-else>{{ row.workDay ? "是" : "否" }}</el-tag>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="{ row }">
          <el-tag type="primary" v-if="row.businessTripDay">{{
            row.businessTripDay ? "是" : "否"
          }}</el-tag>
      ...
      >
        <template slot-scope="{ row }">
          <el-tag type="primary" v-if="row.absenceDay">{{
            row.absenceDay ? "是" : "否"
          }}</el-tag>
          <el-tag type="danger" v-else>{{
            row.absenceDay ? "是" : "否"
  ...
  methods: {
    getList() {
      ...
      if (this.$store.state.user.right == 0) {
        getOneCheckStat(this.$store.state.user.name).then((response) => {
        ...
        getDailyCheckList().then((response) => {
          this.list = response.data;
          this.allData = response.data;
          this.total = this.list.length;
    ...
    uploadExcel() {
      ...
      for (let data of this.tableData) {
        ...
        emp["empID"] = data[keys[0]];
        emp["checkDate"] = data[keys[1]];
        emp["overtimeHour"] = data[keys[2]];
        emp["workDay"] = data[keys[3]];
        emp["businessTripDay"] = data[keys[4]];
        emp["absenceDay"] = data[keys[5]];
        emp["lateDay"] = data[keys[6]];
        if (
          emp["empID"] === undefined ||
          emp["checkDate"] === undefined ||
          emp["overtimeHour"] === undefined ||
          emp["workDay"] === undefined ||
          emp["businessTripDay"] === undefined ||
          emp["absenceDay"] === undefined ||
          emp["lateDay"] === undefined
        ) {
          this.$notify({
            title: "Error",
            message: "请检查数据是否完整",
